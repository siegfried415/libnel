...
* [第14章 使用libnel提供的API生成引擎](#使用libnel提供的API生成引擎)
  ...
  * [§14.3 调用NEL提供的API生成事件分析引擎](#调用NEL提供的API生成事件分析引擎)
...
一般来说，使用平台实现某个应用级引擎，协议开发者首先必须使用C语言来书写协议分析过程中所需的处理函数（如从数据包中分析出协议实体的函数、以及对某些协议约束关系进行检验的函数），这些处理函数经过编译后生成目标代码（\*.o），然后和引擎库（libnel.so）链接后就形成了可执行的二进制文件。

其次，协议开发者需要使用NEL来书写协议规则，协议规则的主要目的是将C语言中定义的协议实体变换为NEL所能够理解的事件，这是通过event关键字来实现的，这些用NEL来描述的协议事件将为攻击规则开发者提供一个事件编程接口。

最后，攻击规则开发者将根据自己对于攻击特征的了解，使用上述事件编程接口对攻击进行定义（比如定义某个事件内部变量之间的约束关系，或者定义多个事件之间的约束关系），在此过程中，规则开发者依然可以使用C语言中的各种语言实体。

在这种模式下，C语言是一个宿主语言，而NEL是一个寄生语言，NEL平台提供了无缝地使用宿主语言中的语言实体的功能，使得NEL的作者能够方便地利用C语言的遗产代码。

生成的引擎可以以"解释模式"和"编译模式"这两种方式下执行，在解释模式下，事件引擎直接根据语法树进行解释执行。在编译模式下，将调用编译后的函数而不再使用语法树，编译模式的分类效率要比解释模式高出一个数量级。目前开源的libnel只提供解释模式。
...
在NEL中可以使用C语言中定义的函数，也可以使用NEL中定义的函数，另外开发者还可以使用libnlib.so中定义的NEL库函数（将在[NEL库函数](#NEL库函数)介绍）。在本章中，我们重点讲述NEL函数的定义和调用，在NEL中能够定义函数的能力提高了NEL的灵活性。
...
在foo.nel中，我们也定义了两个*init*入口，在这两个入口之间，我们使用了<em>#include</em>将bar.nel引入。两个文件的*init*和*fini*的执行顺序如注释所示：
<pre><em>
init{
...
NEL提供的运行程序库包含字符串流函数、正则表达式匹配函数、精确串匹配函数、字符串函数和缓冲区攻击检测函数等。用户使用这些函数，可以加快制作规则的速度。
...
参数：
stream : nel_stream_alloc创建的nel_stream；
...
功能：根据模式串数组生成一个struct match_info数据结构，用于以后的正则表达式匹配函数（buf_match以及stream_match）
参数：
...
第一条规则意味着 *ANY* 可以不经任何其他实际存在的事件的触发而直接规约而成，第二条规则意味着 *ANY* 可以包含1个或者任意多个 *any* 的连续出现，NEL分析器看到这样的定义后，则将删除第一条产生式，而且如果存在产生式：
<pre><em>
A : B ANY C;
</em></pre>
...
这时我们对于需要传递给事件分析引擎的协议事件已经有所了解，最后一步，就可以在协议分析代码中增加对"事件分析引擎"和"事件连接"的调用代码。从而最终完成协议分析引擎。这部分将在[调用NEL提供的API生成事件分析引擎](#调用NEL提供的API生成事件分析引擎)中详细介绍。

最后，我们将在[引擎的运行过程](#引擎的运行过程)中讨论引擎的动态运行过程，用以加深对各个过程的作用的理解。
...
2.  stab_file_parse函数
<pre>
int stab_file_parse(struct nel_eng *eng，char *fname);
</pre>
...
3.  nel_file_parse函数
<pre>
int nel_file_parse(struct nel_eng *eng，char *fname);
</pre>
...
一般地，应用开发人员只需要将struct nel_env定义为协议连接表项的一个字段，而无须理解struct nel_env的内部结构。
...
如果用户给定的事件名是atom事件，那么idof返回的id将大于0；如果是一个抽象事件，那么idof返回的id将小于0；如果指定的事件不存在，或者指定的事件虽然存在，但是对于引擎来说并没有使用该事件（比如说，*_target* 没有直接或者间接地使用到的事件）将返回0。如果nel_env_analysis调用时提交的事件id为0，这意味着引擎不会分析该事件，对于这样的事件，即使不提交引擎，也不会造成什么问题。
